...
test("formatMetrics includes all fields", () => {
	assert.equal(
		formatMetrics({ numTurns: 3, totalCostUsd: 0.5432, promptTokens: 62000, contextPercent: 6.2, durationMs: 12500 }),
		"3 turn(s) · $0.543 · 62k tok · 6.2% ctx · 13s",
	);
});

test("formatMetrics omits unknown fields", () => {
	assert.equal(formatMetrics({ numTurns: 1, totalCostUsd: 0.5, promptTokens: 0, contextPercent: null, durationMs: null }), "1 turn(s) · $0.500");
});
...
test("parseTranscriptMeta defaults on unparseable head", () => {
	assert.deepEqual(parseTranscriptMeta("garbage"), { mode: "delegate", cost: 0, sessionId: null });
	assert.deepEqual(parseTranscriptMeta(""), { mode: "delegate", cost: 0, sessionId: null });
});
...
test("buildReportContent assembles header, body and footers", () => {
	...
	assert.ok(content.includes("resume: `/claude --resume=abc-123 <prompt>`"));
});

test("buildReportContent omits footers when absent", () => {
	const content = buildReportContent({ mode: "review", metrics: "1 turn(s)", body: "x" });
