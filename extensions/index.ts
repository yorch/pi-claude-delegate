...
import { type ClaudeResult, DEFAULT_TIMEOUT_MS, runClaude } from './run-claude.ts';
...
async function delegate(
  ...
  if (!template) {
    throw new Error(`unknown claude_delegate mode "${mode}". Available: ${[...templates.keys()].sort().join(', ')}`);
  }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      model: Type.Optional(
        Type.String({ description: 'Claude model (e.g. sonnet, opus). Defaults to template/config.' }),
      ),
      ...
      allowDangerous: Type.Optional(
        Type.Boolean({
          description: 'Escalate to bypassPermissions (all tools, no prompts). Only with explicit user approval.',
        }),
      ),
    }),
  ...
  pi.registerCommand('claude', {
    ...
    handler: async (args, ctx) => {
      ...
      const dangerous =
        parsed.mode === 'general' ? false : (template?.permissionMode ?? 'acceptEdits') === 'bypassPermissions';

      if (!resolved) {
        if (parsed.mode) {
          ctx.ui.notify?.(
            `/claude ${parsed.mode} <what to do> — give a prompt for the "${parsed.mode}" mode`,
            'warning',
          );
        } else {
      ...
      const pushChip = () => {
        ...
        ctx.ui.setStatus(
          'claude-delegate',
          theme.fg('accent', '●') + theme.fg('dim', ` claude ${mode ?? 'general'}`) + activity,
        );
      };
      ...
      if (cancelled || !result) {
        ...
        if (ctx.hasUI) {
          ctx.ui.notify(
            `claude delegate ${cancelled ? 'cancelled' : 'failed'}: ${message}`,
      ...
      if (ctx.hasUI) {
        ...
        ctx.ui.notify(`claude ${details.mode} done — ${metrics}${resumeHint}` + ` · transcript: ${file}`, 'info');
      } else {
